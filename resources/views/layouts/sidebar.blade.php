...
<section class="sidebar">
    ...
    <div id="left_menu_to_scroll">
        <ul class="sidebar-menu ps ps--active-x ps--active-y tree" data-widget="tree">
            ...
            @endif
            @if (menuPermission('Table Mapping'))
                <li
                    class="parent-menu treeview menu__cidirp_10{{ request()->is('tables*') || request()->is('history*') || request()->is('daily-mapping*') ? ' menu-open active_sub_menu' : '' }}">
                    <a href="#">
                        <iconify-icon icon="solar:chart-2-broken"></iconify-icon>
                        <span class="match_bold">@lang('index.table')</span>
                    </a>
                    
                    <ul class="treeview-menu">                        
                        @if (routePermission('tables.index'))
                            <li class="menu_assign_class {{ request()->routeIs('tables.index') ? ' treeMenuActive' : '' }}" data-menu__cid="irp_10"><a
                                    href="{{ route('tables.index') }}">@lang('index.table')</a>
                            </li>
                        @endif
                        @if (routePermission('daily-mapping.index'))
                            <li class="menu_assign_class {{ request()->routeIs('daily-mapping.index') ? ' treeMenuActive' : '' }}" data-menu__cid="irp_10"><a
                                    href="{{ route('daily-mapping.index') }}">@lang('index.tableMapping')</a>
                            </li>
                        @endif               
                        @if (routePermission('history.index'))
                            <li class="menu_assign_class {{ request()->routeIs('history.index') ? ' treeMenuActive' : '' }}" data-menu__cid="irp_10"><a
                                    href="{{ route('history.index') }}">@lang('index.tableHistory')</a>
                            </li>
                        @endif               
                    </ul>
                </li>

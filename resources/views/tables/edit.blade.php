            ...
            <form action="{{ route('tables.update', $table) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('floor') is-invalid @enderror" id="floor" name="floor" required>
                        ...
                        <option value="1st floor" {{ old('floor', $table->floor) == '1st floor' ? 'selected' : '' }}>1st floor</option>
                        <option value="2nd floor" {{ old('floor', $table->floor) == '2nd floor' ? 'selected' : '' }}>2nd floor</option>
                        <option value="3rd floor" {{ old('floor', $table->floor) == '3rd floor' ? 'selected' : '' }}>3rd floor</option>
                  {{ old('floor', $table->floor) == '3rd floor' ? 'selected' : '' }}>3rd floor</option>
                    </select>
                    @error('floor')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('seats') is-invalid @enderror" id="seats" name="seats" value="{{ old('seats', $table->seats) }}" min="1" required>
